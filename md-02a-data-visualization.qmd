...
3.  Do not display (ignore) the outliers in the plot. **Note: Use a search engine or an AI tool to find the solution and add the link to the solution you have found.**
    ...
    Solution found at this link: <https://www.geeksforgeeks.org/ignore-outliers-in-ggplot2-boxplot-in-r/>
...
3.  Run the code contained in the code chunk and fix any errors.
    ...
    ```{r}
    ggplot(data=gapminder_summary_2007, mapping = aes(x = continent, y = count, fill = continent))+
      geom_col()
    ```
...
3.  Run the code contained in the code chunk and fix any errors.
    ...
    ```{r}
    ggplot(data=gapminder_2007, mapping = aes(x = continent, fill = continent))+
      geom_bar()
    ```

4.  The plot is identical to the plot created with `geom_col()`. Why? What does the `geom_bar()` function do? Write your text here: It appears that the geom_bar plot automatically normalizes the data in reference to the rest of the dataset.
...
6.  Describe how the `geom_histogram()` function is similar to the `geom_bar()` function.
    ...
    The histogram shows the frequency of life expectancies broken up by country. You could do the same thing with the geom_bar chart but you would have to tell it how to break the bars apart rather than it automatically doing so based on frequency of occurrences. We more or less did this (but not by life expectancy) by putting count on the y axis. Currently the bar plots above show the frequency of continents in the dataset.

7.  What happens by adding the 'breaks' argument? Play around with the numbers inside of `seq()` to see what changes. Describe here what you observe: The first two numbers represent the upper and lower bounds for the x axis. The third arguement is the range of each bin, so if it is 10 each bin is 10 years, ie 10-20, 20-30, etc. Smaller final bin ranges allow for a greater level of detail to be conveyed by this graph. Particularly if you want to see the pattern of colors.

```{r}
ggplot(data=gapminder_2007, mapping = aes(x = lifeExp, fill = continent))+
  geom_histogram(col="grey30", breaks=seq(35,85,2.5))
```
...
4.  What is stored in the `country_colors` object? Find out by executing the object in the Console (type it to the Console and hit enter). Do the same again, but with a question mark `?country_colors`.
    ...
    There is a color code for each country, grouped into siilar shades by continent. By running the object with a question mark we can see the R documentation for this package of color schemes.
